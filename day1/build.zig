...
pub fn build(b: *std.Build) void {
    ...
    // such a dependency.
    const run1_cmd = b.addRunArtifact(part1_exe);
    const run2_cmd = b.addRunArtifact(part2_exe);
    ...
    const run1_step = b.step("one", "Run the part 1 executable");
    run1_step.dependOn(&run1_cmd.step);

    const run2_step = b.step("two", "Run the part 2 executable");
